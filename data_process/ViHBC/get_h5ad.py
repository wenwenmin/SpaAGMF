...
def gene_selection(adata_list, save_dir, n_genes=3000):
    ...
    for adata in adata_list:
        ...
        print(f"Saved {name} shape: {adata.shape},cancer: {adata.obs['cancer'].mean() * 100:.1f}%")
...
def process_tgt(data_dir, name, uni):
    ...
    label_path = data_dir / 'truth.txt'

    # 2. load the adata
    adata = sc.read_visium(data_dir)
    label = pd.read_csv(label_path, sep='\t', header=None, index_col=0)
    label['cancer'] = label[1].apply(lambda x: 0 if ('healthy' in x.lower() or 'tumor_edge' in x.lower()) else 1)
    # 6. Create Precessed data
    adata.obs = adata.obs[[]]
    adata.obs['cancer'] = label['cancer']
    key = list(adata.uns['spatial'].keys())[0]
    scale =  adata.uns['spatial'][key]['scalefactors']['tissue_hires_scalef']
    adata.uns["spot_diameter"] = adata.uns['spatial'][key]['scalefactors']['spot_diameter_fullres'] * scale
...
if __name__ == '__main__':
    ...
    tgt_dir = Path(r'xxx') # Modify it to your ViHBC data directory (Human_Breast_Cancer)
    tgt_names = ['ViHBC']
    # 2. Define the save directory
    save_dir = Path(__file__).parents[2] / 'data/STHBC2ViHBC/'
